...
for repo in ${independent[@]}; do
    ...
    if [[ $action == delete ]]; then
        ...
        if [[ $repo == @(kernel_manifest-5.10|kernel_manifest-5.15|kernel_manifest-6.1|kernel_manifest-caimito|kernel_manifest-gs|kernel_manifest-redbull|kernel_manifest-zuma) ]]; then
            git checkout -B tmp
            sed -i s%refs/heads/$branch%refs/tags/$tag_name% default.xml
            git commit default.xml -m $tag_name
            git push -fu origin tmp
        ...
        if [[ $repo != (device_google_caimito-kernels_6.1|kernel_common-6.1|kernel_manifest-caimito|script) ]]; then
            git checkout $base_branch
            git checkout -B $branch
            git push -fu origin $branch
        fi
